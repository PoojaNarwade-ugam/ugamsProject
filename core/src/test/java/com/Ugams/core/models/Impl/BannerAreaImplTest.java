package com.Ugams.core.models.Impl;

import com.Ugams.core.models.BannerArea;
import com.Ugams.core.models.Price;
import io.wcm.testing.mock.aem.junit5.AemContext;
import io.wcm.testing.mock.aem.junit5.AemContextExtension;
import org.apache.sling.api.resource.Resource;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.extension.ExtendWith;
import org.mockito.junit.jupiter.MockitoExtension;

import static org.junit.jupiter.api.Assertions.*;
@ExtendWith({AemContextExtension.class})


class BannerAreaImplTest {
    private final AemContext aemContext=new AemContext();
    BannerArea bannerArea;

    @BeforeEach
    void setUp() throws Exception{
       // aemContext.addModelsForClasses(PriceImpl.class);
        aemContext.load().json("/BannerArea.json","/content");
    }

    @Test
    void getBannerAreaTitle() {
        Resource resource=aemContext.currentResource("/content/banner");
        bannerArea= resource.adaptTo(BannerArea.class);
        final String expected="THIS IS ME";
        String actual=bannerArea.getBannerAreaTitle();
        assertEquals(expected,actual);
    }

    @Test
    void getBannerAreaHeading() {
        Resource resource=aemContext.currentResource("/content/banner");

        bannerArea=resource.adaptTo(BannerArea.class);
        final String expected="PHILIP GILBERT";
        String actual=bannerArea.getBannerAreaHeading();
        assertEquals(expected,actual);
    }

    @Test
    void getBannerAreaDescription() {
        Resource resource=  aemContext.currentResource("/content/banner");
        bannerArea=resource.adaptTo(BannerArea.class);
        final String expected="You will begin to realise why this exercise is called the Dickens Pattern with reference to the ghost showing Scrooge some different futures.";
        String actual=bannerArea.getBannerAreaDescription();
        assertEquals(expected,actual);
    }

    @Test
    void getBannerAreaButtonTitle() {
        Resource resource= aemContext.currentResource("/content/banner");
        bannerArea=resource.adaptTo(BannerArea.class);
        final String expected="DISCOVER NOW";
        String actual=bannerArea.getBannerAreaButtonTitle();
        assertEquals(expected,actual);

    }

    @Test
    void getImg() {
        Resource resource=aemContext.currentResource("/content/banner");
        bannerArea=resource.adaptTo(BannerArea.class);
        assertEquals("/content/dam/ugams/hero-img.png",bannerArea.getImg());
    }

    @Test
    void getPathValue() {
        Resource resource=aemContext.currentResource("/content/banner");
        bannerArea=resource.adaptTo(BannerArea.class);
        assertEquals("/content/ugams/us/en/about",bannerArea.getPathValue());
    }
}